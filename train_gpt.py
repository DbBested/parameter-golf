...
class Hyperparameters:
    ...
    ttt_unfreeze_last_n = int(os.environ.get("TTT_UNFREEZE_LAST_N", 2))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.vrl_gate: nn.Parameter | None = None  # created by GPT.__init__ only for VRL layers
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, v_residual: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.use_vrl and v_residual is not None:
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=not _FA3_NEEDS_GRAPH_BREAK)
    with torch.inference_mode():
...
def score_first_ttt(
    ...
    For each chunk: score tokens first (record loss), then train on scored tokens.
    Only tokens that have ALREADY been graded are used for training."""
    seq_len = args.train_seq_len
    chunk_size = args.ttt_chunk_tokens
    num_layers = args.num_layers
    total_tokens = val_tokens.numel() - 1
    log0 = lambda s: print(s, flush=True) if rank == 0 else None
    # Freeze everything except last N blocks + norms + lm_head
    unfreeze_block_start = num_layers - args.ttt_unfreeze_last_n
    ...
    for name, param in model.named_parameters():
        ...
        if "final_norm" in name:
            ...
            should_unfreeze = True
        elif "tok_emb" in name and args.tie_embeddings:
            should_unfreeze = True  # tied embeddings = lm_head
        elif "blocks." in name:
            parts = name.split(".")
            if len(parts) >= 2 and parts[1].isdigit():
                layer_idx = int(parts[1])
                if layer_idx >= unfreeze_block_start:
                    should_unfreeze = True
        if should_unfreeze:
    ...
    log0(f"ttt:unfrozen {unfrozen_count:,}/{total_params:,} params "
         f"(blocks {unfreeze_block_start}-{num_layers-1} + norms + head)")
    # Setup optimizer with cosine decay
    num_chunks = (total_tokens + chunk_size - 1) // chunk_size
    total_ttt_steps = num_chunks * args.ttt_epochs
    optimizer = torch.optim.AdamW(unfrozen_params, lr=args.ttt_lr, weight_decay=0.0)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_ttt_steps, eta_min=0.0)
    # Score-first TTT: for each chunk, score then train
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    chunk_starts = list(range(0, total_tokens, chunk_size))
    # ALL ranks process ALL chunks in same order to keep models synchronized
    t_ttt_start = time.perf_counter()
    for ci, cs in enumerate(chunk_starts):
        ce = min(cs + chunk_size, total_tokens)
        chunk_tokens = val_tokens[cs:ce + 1].to(device=device, dtype=torch.int64)
        chunk_len = ce - cs
        # Phase 1: SCORE — only rank that owns this chunk records the loss
        # (distribute scoring across ranks for speed, but all ranks train on all chunks)
        my_score = (ci % world_size == rank)
        model.eval()
        with torch.inference_mode():
            for si in range(0, chunk_len, seq_len):
                end_i = min(si + seq_len, chunk_len)
                x = chunk_tokens[si:end_i].unsqueeze(0)
                y = chunk_tokens[si + 1:end_i + 1].unsqueeze(0)
                ...
                if my_score:
                    ...
                    nll = F.cross_entropy(
                        logits[:, :y.size(1)].reshape(-1, logits.size(-1)).float(),
                        y.reshape(-1),
                        reduction="none",
                    )
                    ...
                    tgt = y.reshape(-1)
                    prev = x[:, :y.size(1)].reshape(-1)
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
        # Phase 2: TRAIN — ALL ranks train on this chunk (keeps models synchronized)
        model.train()
        for epoch in range(args.ttt_epochs):
            for si in range(0, chunk_len, seq_len):
                ...
                scheduler.step()
        if ci % 10 == 0:
            elapsed = time.perf_counter() - t_ttt_start
            log0(f"ttt:chunk {ci}/{len(chunk_starts)} elapsed:{elapsed:.0f}s")
    # Gather scores across ranks
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
    ...
    elapsed = time.perf_counter() - t_ttt_start
    log0(f"ttt:done val_loss:{val_loss:.4f} val_bpb:{ttt_bpb:.4f} time:{elapsed:.0f}s")
    # Freeze all params again after TTT
    for param in model.parameters():
        param.requires_grad_(False)
    return val_loss, ttt_bpb
def eval_val_sliding_with_temperature(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
    world_size: int,
    ...
    """Sliding window eval with temperature scaling on logits."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
...
def gptq_quantize(w: Tensor, H: Tensor, clip_range: int = 31, block_size: int = 128) -> tuple[Tensor, Tensor]:
    """Full GPTQ (IST-DASLab algorithm): Hessian-calibrated column-wise quantization.
    w: [out_features, in_features], H: [in_features, in_features] (Hessian = X^T X).
    Returns quantized weights and per-row scales."""
    W = w.float().clone()
    nrows, ncols = W.shape
    H = H.float().clone()
    ...
    H[dead, dead] = 1
    W[:, dead] = 0
    # Activation order: sort by descending Hessian diagonal (quantize most important first)
    perm = torch.argsort(torch.diag(H), descending=True)
    W = W[:, perm]
    H = H[perm][:, perm]
    # Dampen and compute upper Cholesky factor of H^{-1}
    ...
    # Per-row scale (computed on reordered W, best percentile)
    best_s = None
    best_err = float('inf')
    ...
    # Block-wise quantization with error compensation (GPTQ Algorithm 1)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    # Build name -> module path mapping for GPTQ Hessian lookup
    ...
    # Magnitude pruning: zero out smallest N% of quantized weights to improve compression
...
def main() -> None:
    ...
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=not _FA3_NEEDS_GRAPH_BREAK)
    ...
    if rank == 0:
        log0("gptq:collecting Hessians (128 batches)...")
        hooks = []
        ...
        base_model.eval()
        with torch.no_grad():
            n_cal = min(128, len(val_tokens) // 2048)
    ...
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=not _FA3_NEEDS_GRAPH_BREAK)
    torch.cuda.synchronize()
    ...
    # Score-first TTT (legal: only trains on already-graded tokens)
    if args.ttt_enabled:
        ...
        torch.cuda.synchronize()
        log0(f"ttt:score_first val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} "
             f"time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms")
        log0(f"ttt:score_first_exact val_loss:{ttt_loss:.8f} val_bpb:{ttt_bpb:.8f}")
        # Re-score with temperature calibration (T=0.98 corrects TTT overconfidence)
        if args.ttt_temperature != 1.0:
            torch.cuda.synchronize()
            t_rescore = time.perf_counter()
            ttt_t_loss, ttt_t_bpb = eval_val_sliding_with_temperature(
                args, eval_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            ...
            torch.cuda.synchronize()
            log0(f"ttt:rescore_T{args.ttt_temperature} val_loss:{ttt_t_loss:.4f} val_bpb:{ttt_t_bpb:.4f} "
                 f"stride:{args.eval_stride} time:{1000.0 * (time.perf_counter() - t_rescore):.0f}ms")
            log0(f"ttt:rescore_exact val_loss:{ttt_t_loss:.8f} val_bpb:{ttt_t_bpb:.8f}")
    if distributed:
